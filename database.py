...
INFO_ESPECIES = {
    'bovino_corte': {'racas': ['nelore', 'angus', 'guzera', 'brahman'], 'peso_jovem': 12.0, 'peso_adulto': 18.0, 'gestacao': 280, 'ganho_dia': 0.8, 'dieta': 'pasto'},
    'bovino_leite': {'racas': ['girolando'], 'peso_jovem': 10.0, 'peso_adulto': 15.0, 'gestacao': 280, 'ganho_dia': 0.6, 'dieta': 'pasto'},
    'equino': {'racas': ['cavalo'], 'peso_jovem': 15.0, 'peso_adulto': 25.0, 'gestacao': 340, 'ganho_dia': 0.5, 'dieta': 'pasto'},
    'suino': {'racas': ['porco'], 'peso_jovem': 2.0, 'peso_adulto': 6.0, 'gestacao': 114, 'ganho_dia': 0.3, 'dieta': 'racao'},
    'ave': {'racas': ['galinha', 'pato', 'peru'], 'peso_jovem': 0.05, 'peso_adulto': 0.15, 'gestacao': 21, 'ganho_dia': 0.01, 'dieta': 'racao'},
    'peixe_gigante': {'racas': ['pirarucu', 'surubim', 'pintado', 'cachara'], 'peso_jovem': 1.0, 'peso_adulto': 4.0, 'gestacao': 0, 'ganho_dia': 0.1, 'dieta': 'racao'},
    'peixe_medio': {'racas': ['tambaqui', 'pacu', 'matrinxa', 'tucunare', 'curimata', 'piau', 'jaraqui'], 'peso_jovem': 0.05, 'peso_adulto': 0.2, 'gestacao': 0, 'ganho_dia': 0.02, 'dieta': 'racao'}
}
...
class Lote(db.Model):
    ...
    # ---------------------------------------------
    # INFRAESTRUTURAS INDIVIDUAIS DE ALIMENTAÇÃO
    # ---------------------------------------------
    tem_cocho = db.Column(db.Boolean, default=False) # Cocho de Sal
    qtd_sal_cocho = db.Column(db.Float, default=0.0) 
    
    tem_cocho_racao = db.Column(db.Boolean, default=False) # Linha de Ração
    qtd_racao_cocho = db.Column(db.Float, default=0.0) 
    
    # Campos que ficaram do teste anterior (Mantidos para o banco não dar erro)
    ...
    def processar_biologia_animal(self, ambiente):
        ...
        if self.fome > 50 or self.saude < 40:
            ...
            if self.peso < peso_maximo_genetico:
                # Fator genético individual: cada animal engorda num ritmo único!
                fator_individual = random.uniform(0.75, 1.25)
...
def popular_mapa_inicial():
    if Propriedade.query.first(): return
    # (Código padrão mantido oculto por brevidade)
